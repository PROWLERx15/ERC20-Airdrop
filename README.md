...
The Airdrop smart contract allows eligible users to claim ERC-20 tokens based on a Merkle tree proof and a digital signature verification. It ensures that only valid claimants, pre-determined by the airdrop organizer, can receive tokens.
...
```bash
git clone https://github.com/PROWLERx15/ERC20-Airdrop
cd ERC20-Airdrop
...
Then, retrieve the `root` (there may be more than 1, but they will all be the same) from `script/target/output.json` and paste it in the `Makefile` as `ROOT` (for zkSync deployments) and update `ROOT` in `DeployAirdrop.s.sol` for Ethereum/Anvil deployments.
...
make anvil
make deploy
# Copy the Token address & Airdrop contract address
```
Copy the Token and Aidrop contract addresses and paste them into the `AIRDROP_ADDRESS` and `TOKEN_ADDRESS` variables in the `MakeFile`
...
Retrieve the signature bytes outputted to the terminal and add them to `Interactions.s.sol` *making sure to remove the `0x` prefix*. 
